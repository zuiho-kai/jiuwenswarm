...
class JiuWenSwarmDeepAdapter:
    ...
    def _build_cua_task_tool(self, agent_id: str) -> Any | None:
        ...
        def current_permissions() -> dict[str, Any]:
            # Resolve at execution time: queued tasks must see permission reloads.
            from jiuwenswarm.agents.harness.common.rails.permissions.permission_compose import (
                compose_host_effective_permissions,
            )
            from jiuwenswarm.agents.harness.common.rails.permissions.permissions_layers import (
            ...
            active = getattr(self, "_active_code_config", None)
            current_config = active() if callable(active) else get_config()
            session_id = str(getattr(self, "_parent_session_id", None) or "").strip() or None
            return compose_host_effective_permissions(
                global_permissions=current_config.get("permissions") or {},
                user_permissions=load_user_permissions(),
                session_permissions=load_session_permissions(session_id),
                session_id=session_id,
            )
        ...
        return build_cua_task_tool(
            ...
            agent_id=agent_id,
            language=self._resolve_runtime_language(),
            permissions_getter=current_permissions,
        )
